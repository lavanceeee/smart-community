<template>
    <div class="p-6 space-y-6">
        <!-- 页面头部 -->
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">事项投诉管理</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">处理用户提交的事项投诉</p>
        </div>
            ...
            <el-form :model="queryForm" inline class="flex flex-wrap gap-4">
                <el-form-item label="投诉类型" class="!mb-0">
                    ...
                    >
                        ...
                        <el-option label="垃圾处理" value="垃圾处理" />
            ...
            >
                ...
                <el-table-column label="投诉类型" width="120">
                    <template #default="{ row }">
                        <el-tag type="warning" size="small">{{ row.complaintType }}</el-tag>
                    </template>
                </el-table-column>
                ...
                <div class="bg-slate-50 dark:bg-slate-700 rounded-lg p-4">
                    <div class="grid grid-cols-2 gap-3 text-sm">
                        <div>
                            ...
                            <span class="font-medium">{{ currentComplaint.userId }}</span>
                        </div>
                        <div>
                            <span class="text-slate-500">投诉类型：</span>
                            <el-tag type="warning" size="small">{{ currentComplaint.complaintType }}</el-tag>
                        </div>
                        <div class="col-span-2">
                            <span class="text-slate-500">位置：</span>
                            <span class="font-medium">{{ currentComplaint.location }}</span>
                        </div>
                        <div class="col-span-2">
                            <span class="text-slate-500">描述：</span>
                            <p class="mt-1 text-slate-700 dark:text-slate-200">{{ currentComplaint.description }}</p>
                        </div>
                        <div>
                            <span class="text-slate-500">提交时间：</span>
                            <span class="font-medium">{{ currentComplaint.createTime }}</span>
                        </div>
                    </div>
                </div>
...
import { queryComplaintListApi, handleComplaintApi } from '~/utils/api'
...
const fetchComplaintList = async () => {
    ...
    try {
        ...
        const response = await queryComplaintListApi({
            ...queryForm,
            complaintType: queryForm.complaintType || undefined,
        ...
        if (response.code === 200 && response.data) {
            ...
            statistics.processing = complaintList.value.filter(c => c.status === 1).length
            statistics.completed = complaintList.value.filter(c => c.status === 2).length
        } else {
...
// 重置
const handleReset = () => {
    queryForm.complaintType = undefined
...
// 处理投诉
const handleProcess = (complaint: Complaint) => {
    currentComplaint.value = complaint
    processForm.status = complaint.status === 0 ? 1 : complaint.status
    processForm.handleResult = complaint.handleResult || ''
...
const handleSubmitProcess = async () => {
    ...
    try {
        await handleComplaintApi(currentComplaint.value.complaintId, {
