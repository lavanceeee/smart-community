<template>
  <div class="max-w-[1240px] mx-auto px-4 py-6">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 lg:gap-4">
      ...
      >
        ...
        <div class="aspect-square overflow-hidden bg-slate-100 dark:bg-slate-800 relative">
          ...
          <div class="absolute inset-x-0 bottom-0 bg-black/5 py-1 text-center opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div class="p-3 flex-1 flex flex-col">
          ...
          <h3 class="text-sm font-medium text-slate-800 dark:text-slate-200 line-clamp-2 leading-snug mb-2 min-h-[2.5rem]">
            <span 
              v-if="item.platform === '天猫'" 
              class="bg-red-500 text-white text-[9px] px-1 rounded mr-1 inline-block transform translate-y-[-1px]"
            >天猫</span>
            <span 
              v-else-if="item.platform === '天猫国际'" 
              class="bg-purple-600 text-white text-[9px] px-1 rounded mr-1 inline-block transform translate-y-[-1px]"
            >天猫国际</span>
            {{ item.name }}
          </h3>
          ...
          <div class="flex flex-wrap gap-1 mb-2">
            <span 
              v-if="item.promotion" 
              class="text-[#ff5000] text-[10px] font-bold"
            >{{ item.promotion }}</span>
            <span 
              v-for="tag in item.tags.slice(0, 2)" 
              :key="tag"
              class="border border-[#ff5000]/30 text-[#ff5000] text-[9px] px-1 rounded leading-tight"
            >{{ tag }}</span>
          </div>
          ...
          <div class="mt-auto flex items-baseline justify-between gap-1 flex-wrap">
            <div class="flex items-baseline text-[#ff5000]">
              ...
              <span v-if="item.price.includes('.')" class="text-xs font-bold">.{{ item.price.split('.')[1] || '0' }}</span>
            </div>
    ...
    <div class="mt-12 text-center">
      <button class="px-8 py-2.5 rounded-full border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-sm hover:border-[#ff5000] hover:text-[#ff5000] transition-colors">
        加载更多
      </button>
    </div>
  </div>
