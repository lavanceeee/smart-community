    ...
    <div
        ...
        <div class="flex gap-4 h-[320px]">
            ...
            <div class="flex-1 flex flex-col gap-3 min-w-0">
                <!-- 顶部：模块标题 + 搜索框 -->
                <div class="flex items-center justify-between gap-4 h-11 shrink-0">
                    <div class="text-lg font-bold text-slate-700 dark:text-slate-200 px-1">社区服务</div>
                ...
                <div class="flex-1 relative rounded-xl overflow-hidden group">
                    ...
                    <!-- 遮罩层 -->
                    <div class="absolute inset-0 bg-gradient-to-r from-blue-600/90 via-blue-600/70 to-transparent">
                    </div>

                    <!-- 文字内容 -->
                    <div class="absolute inset-0 flex flex-col justify-center px-8 z-10">
                        <h2 class="text-white text-3xl font-bold leading-tight mb-4 drop-shadow-lg whitespace-pre-line">
                    ...
                    <div class="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5 z-20">
                        <button v-for="(_, index) in bannerList" :key="index" @click="goToSlide(index)"
                            class="group/dot p-1">
                            <span
                                class="block h-1 rounded-full transition-all duration-300 bg-white/40 group-hover/dot:bg-white/80"
                                :class="index === currentIndex ? 'w-6 bg-white' : 'w-3'">
                            </span>
                        </button>
                    </div>
                </div>
