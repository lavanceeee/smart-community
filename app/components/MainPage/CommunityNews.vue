<template>
  <div class="bg-white dark:bg-white/5 rounded-xl p-5 border border-slate-100 dark:border-white/5 shadow-sm">
    ...
    <div class="flex flex-col md:flex-row gap-6">
        ...
        <el-carousel trigger="click" height="340px" :interval="5000" arrow="hover">
          <el-carousel-item v-for="item in bannerList" :key="item.id">
            <div class="relative w-full h-full cursor-pointer">
              ...
              <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-5 pt-16">
                <span class="bg-[#ff5000] text-white text-[10px] px-1.5 py-0.5 rounded mb-1.5 inline-block font-medium">热点</span>
      ...
      <div class="flex-1 flex flex-col justify-between gap-6 min-w-0 py-1">
        ...
        <div class="flex flex-col gap-3">
          <div class="flex items-center justify-between border-b border-slate-100 dark:border-white/5 pb-2">
            ...
            <NuxtLink to="/news/politics" class="text-[11px] text-slate-400 hover:text-[#d93025] flex items-center">
              更多 <Icon name="lucide:chevron-right" size="12" />
            </NuxtLink>
          </div>
          
          <ul class="flex flex-col gap-3">
            <li v-for="(news, i) in politicsList" :key="i" class="flex items-center justify-between group cursor-pointer">
              <div class="flex items-center gap-2 min-w-0 pr-4">
                <span class="w-1.5 h-1.5 rounded-full bg-slate-200 group-hover:bg-[#d93025] transition-colors shrink-0 mt-0.5"></span>
                <span class="text-[13px] text-slate-700 dark:text-slate-300 truncate group-hover:text-[#d93025] transition-colors leading-normal">
                  {{ news.title }}
                </span>
                <span v-if="i === 0" class="text-[9px] text-[#d93025] border border-[#d93025]/30 px-1 rounded-sm scale-90 origin-left shrink-0">最新</span>
              </div>
        ...
        <div class="flex flex-col gap-3">
          ...
          <ul class="flex flex-col gap-3">
            <li v-for="(news, i) in affairsList" :key="i" class="flex items-center justify-between group cursor-pointer">
              <div class="flex items-center gap-2 min-w-0 pr-4">
                <span class="w-1.5 h-1.5 rounded-full bg-slate-200 group-hover:bg-[#1a73e8] transition-colors shrink-0 mt-0.5"></span>
                <span class="text-[13px] text-slate-700 dark:text-slate-300 truncate group-hover:text-[#1a73e8] transition-colors leading-normal">
                  {{ news.title }}
                </span>
              </div>
              <span class="text-xs text-slate-400 shrink-0 tabular-nums font-medium tracking-tight">
