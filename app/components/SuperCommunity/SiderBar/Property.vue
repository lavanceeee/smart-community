<template>
    ...
    >
        <NuxtLink v-if="hasPermission('property:view')" to="/superCommunity/property/fee"
            ...
            :class="isActive('/superCommunity/property/fee') ? 'bg-blue-600 text-white shadow-md shadow-blue-900/20' : 'hover:bg-slate-800 hover:text-white'">
            <Icon name="lucide:receipt" size="18" class="mr-3 flex-shrink-0"
                :class="isActive('/superCommunity/property/fee') ? 'text-white' : 'text-slate-400 group-hover:text-white'" />
            物业费管理
        </NuxtLink>
